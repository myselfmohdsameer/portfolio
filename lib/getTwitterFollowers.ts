...
export async function getTwitterFollowers(): Promise<SocialFollowers> {
  ...
  let response = await fetch(
    `https://api.twitter.com/2/users/3066480834/followers?max_results=1000`,
    {
      headers: {
  ...
  while (data.meta?.next_token) {
    response = await fetch(
      `https://api.twitter.com/2/users/3066480834/followers?max_results=1000&pagination_token=${data.meta.next_token}`,
      {
        headers: {
          Authorization: `Bearer ${process.env.TWITTER_BEARER_TOKEN}`
        }
